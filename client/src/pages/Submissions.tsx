        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {submissions.map((sub) => (
            <div
              ...
              className="group bg-white rounded-[2.5rem] p-8 border border-slate-200 hover:border-indigo-600 hover:shadow-2xl hover:shadow-indigo-500/10 active:scale-[0.98] active:bg-slate-50 transition-all cursor-pointer flex flex-col h-full relative overflow-hidden"
            >
              {/* Status Badge - Absolute Positioned */}
              <div className={`absolute top-8 right-8 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm ${getStatusDisplay(sub).color}`}>
              ...
              <div className="flex-1 space-y-6">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2.5 py-1 bg-slate-100 text-slate-900 text-[10px] font-black uppercase tracking-widest rounded-lg">
                      {format(new Date(sub.created_at), 'HH:mm')} Uhr
                    </span>
                  </div>
                  <h3 className="text-2xl md:text-3xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors truncate mb-2 uppercase tracking-tight leading-none">
                    {sub.customer_name || 'Anonymer Einsatz'}
                  </h3>
                  <div className="flex items-center gap-2 text-indigo-900 font-black text-[10px] uppercase tracking-[0.2em] bg-indigo-50 w-fit px-3 py-1 rounded-md">
                ...
                <div className="space-y-4">
                  {sub.customer_email && (
                    <div className="flex items-center gap-3 text-sm text-slate-600 font-bold uppercase tracking-tight">
